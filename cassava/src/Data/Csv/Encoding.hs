...
import qualified ByteString.TreeBuilder as TreeBuilder
...
class Monoid a => IsBuilder a where
    builderByte :: Word8 -> a
    builderByteString :: B.ByteString -> a
    builderString8 :: String -> a

instance IsBuilder Builder where
    builderByte = word8
    builderByteString = byteString
    builderString8 = stringUtf8

instance IsBuilder TreeBuilder.Builder where
    builderByte = TreeBuilder.byte
    builderByteString = TreeBuilder.byteString
    builderString8 = mconcat . fmap TreeBuilder.utf8Char

-- | Encode a single record, without the trailing record separator
-- (i.e. newline).
encodeRecord :: IsBuilder a => Quoting -> Word8 -> Record -> a
encodeRecord qtng delim = mconcat . intersperse (builderByte delim)
                     . map builderByteString . map (escape qtng delim)
                     . V.toList
{-# INLINE encodeRecord #-}

-- | Encode a single named record, without the trailing record
-- separator (i.e. newline), using the given field order.
encodeNamedRecord :: IsBuilder a
                    => Header -> Quoting -> Word8 -> NamedRecord -> a
...
recordSep :: IsBuilder builder => Bool -> builder
recordSep False = builderByte 10 -- new line (\n)
recordSep True  = builderString8 "\r\n"

unlines :: Builder -> [Builder] -> Builder
unlines _ [] = mempty
unlines sep (b:bs) = b <> sep <> unlines sep bs

intersperse :: Monoid m => m -> [m] -> [m]
intersperse _   []      = []
intersperse sep (x:xs)  = x : prependToAll sep xs

prependToAll :: Monoid m => m -> [m] -> [m]
prependToAll _   []     = []
